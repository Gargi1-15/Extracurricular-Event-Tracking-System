...
class Event(db.Model):
    # "event" is a reserved word in PostgreSQL; use "events" on Render.
...
def ensure_postgres_schema():
    """Fix PostgreSQL schema issues common after Render deploys."""
    uri = app.config.get("SQLALCHEMY_DATABASE_URI", "")
    if not uri.startswith("postgresql"):
        return

    db.session.execute(
        text("ALTER TABLE events ADD COLUMN IF NOT EXISTS is_paid BOOLEAN NOT NULL DEFAULT FALSE")
    )
    db.session.execute(text("ALTER TABLE events ADD COLUMN IF NOT EXISTS fee_amount DOUBLE PRECISION"))
    db.session.execute(
        text("ALTER TABLE events ADD COLUMN IF NOT EXISTS payment_method VARCHAR(120)")
    )
    db.session.execute(text("ALTER TABLE events ADD COLUMN IF NOT EXISTS payment_details TEXT"))
    db.session.commit()

    wrong_fk = db.session.execute(
        text(
            """
    ...
    if wrong_fk:
        db.session.execute(text("DROP TABLE IF EXISTS registration CASCADE"))
        db.session.commit()
        db.create_all()


def ensure_event_payment_columns():
    """SQLite-only migration helper for older local databases."""
    uri = app.config.get("SQLALCHEMY_DATABASE_URI", "")
    ...
    columns = {
        row[1]
        for row in db.session.execute(text("PRAGMA table_info(events)")).fetchall()
    }
    if not columns:
        columns = {
            row[1]
    ...
    if "is_paid" not in columns:
        db.session.execute(
            text(f"ALTER TABLE {table_name} ADD COLUMN is_paid BOOLEAN NOT NULL DEFAULT 0")
        )
    if "fee_amount" not in columns:
        db.session.execute(text(f"ALTER TABLE {table_name} ADD COLUMN fee_amount FLOAT"))
    if "payment_method" not in columns:
        db.session.execute(text(f"ALTER TABLE {table_name} ADD COLUMN payment_method VARCHAR(120)"))
    if "payment_details" not in columns:
        db.session.execute(text(f"ALTER TABLE {table_name} ADD COLUMN payment_details TEXT"))
    db.session.commit()
...
def build_event_details_text(event):
    lines = [
        f"Event: {event.title}",
        f"Date: {event.date.strftime('%d %b %Y')}",
        f"Time: {event.start_time.strftime('%H:%M')} - {event.end_time.strftime('%H:%M')}",
...
def notify_registered_students_event_update(event, reason="Event update", custom_message=None):
    """
    Sends email to every student registered for the given event.
    Used for "new updates about that event" notifications.
    """
    sent = 0
    failed = 0

    subject = f"{reason}: {event.title}"
    details_text = build_event_details_text(event)

    for reg in event.registrations:
        greeting = f"Hello {reg.student.name},"
        message_lines = [greeting, ""]

        if custom_message:
            message_lines.extend([custom_message.strip(), "", "—", ""])

        message_lines.append(details_text)
        body = "\n".join(message_lines)

        ok, _ = send_email(reg.student.email, subject, body)
        if ok:
            sent += 1
        else:
            failed += 1
...
def register():
    ...
    try:
        ...
        db.session.rollback()
        flash("Could not register. You may already be registered for this event.", "warning")
...
@app.route("/admin/events/<int:event_id>/send-update-email", methods=["POST"])
def admin_send_event_update_email(event_id):
    if not require_admin():
        return redirect(url_for("admin_login"))
    ...
    if not message:
        flash("Please enter an update message.", "warning")
        return redirect(url_for("admin_event_participants", event_id=event.id))

    reason = subject or "Event update"
    sent, failed = notify_registered_students_event_update(
        event,
        reason=reason,
        custom_message=message,
    )
    if sent:
        flash(f"Update email sent to {sent} registered students.", "info")
    if failed:
...
def admin_edit_event(event_id):
    ...
    if request.method == "POST":
        ...
        if old_event_state != new_event_state and event.registrations:
            sent, failed = notify_registered_students_event_update(event)
            if sent:
                flash(f"Update email sent to {sent} registered students.", "info")
            if failed:
                flash(f"Failed to send {failed} email(s). Check SMTP settings.", "warning")
...
# Ensure tables exist when gunicorn/Render imports the app module.
try:
    init_db()
except Exception as exc:
    import sys
